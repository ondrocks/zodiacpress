...
class ZP_License {
	...
	private function hooks() {
		...
		add_action( 'zodiacpress_settings_tab_top', array( $this, 'license_help_text' ) );
		...
		add_action( 'zp_weekly_scheduled_events', array( $this, 'weekly_license_check' ) );
	...
	public function activate_license() {
		...
		$license = sanitize_text_field( $_POST['zodiacpress_settings'][ $this->item_shortname . '_license_key'] );
	...
	public function deactivate_license() {
		...
		if( ! wp_verify_nonce( $_REQUEST[ $this->item_shortname . '_license_key-nonce'], $this->item_shortname . '_license_key-nonce' ) ) {
			...
			wp_die( __( 'Nonce verification failed', 'zodiacpress' ), __( 'Error', 'zodiacpress' ), array( 'response' => 403 ) );
	...
	public function notices() {
		...
		if ( is_object( $license ) && 'valid' !== $license->license && empty( $showed_invalid_message ) ) {
			...
			if ( empty( $_GET['tab'] ) || 'licenses' !== $_GET['tab'] ) {
				...
				$messages[] = sprintf(
					__( 'You have invalid or expired license keys for ZodiacPress. Please go to the <a href="%s">Licenses page</a> to correct this issue.', 'zodiacpress' ),
					admin_url( 'admin.php?page=zodiacpress-settings&tab=licenses' )
				);
	...
	public function plugin_row_license_missing( $plugin_data, $version_info ) {
		...
		if( ( ! is_object( $license ) || 'valid' !== $license->license ) && empty( $showed_imissing_key_message[ $this->item_shortname ] ) ) {
			echo '&nbsp;<strong><a href="' . esc_url( admin_url( 'admin.php?page=zodiacpress-settings&tab=licenses' ) ) . '">' . __( 'Enter valid license key for automatic updates.', 'zodiacpress' ) . '</a></strong>';
